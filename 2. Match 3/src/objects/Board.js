...
export default class Board {
	 ...
	 * @param {Number} y
	 */
	constructor(x, y, level = 1, width = Board.SIZE, height = Board.SIZE) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	initializeBoard() {
		...
		for (let row = 0; row < this.height; row++) {
			...
			// pick random spot on board for star location
			const starX = getRandomPositiveInteger(0, this.width - 1);
			const starY = getRandomPositiveInteger(0, this.height - 1);

			// For each column in the row...
			for (let column = 0; column < this.width; column++) {
				this.tiles[row].push(this.generateTile(column, row, starX, starY));
			}
		}
	}

	generateTile(x, y, starX = Number.MAX_VALUE, starY = Number.MAX_VALUE) {
		const colourList = [
			TileColour.Beige,
			TileColour.Pink,
			TileColour.Purple,
		...
		const patternRange = [TilePattern.Flat, Math.min(Math.round(this.level / 2.01), TilePattern.Triangle)];
		const colour = pickRandomElement(colourList);

		// If the tile is in the star location, make it a star.
		if (x === starX && y === starY) {
			return new Tile(x, y, colour, TilePattern.Star, this.tileSprites);
		}
		
		// % Chance of star spawning goes up every level	
		const pattern = getRandomPositiveInteger(1, 100) <= this.level ? TilePattern.Star : getRandomPositiveInteger(patternRange[0], patternRange[1]);
	...
	resolveHorizontalMatches() {
		for (let y = 0; y < Board.SIZE; y++) {
			...
			for (let x = 1; x < Board.SIZE; x++) {
				...
				if (this.tiles[y][x].colour === colourToMatch) {
					...
					if (matchCounter >= this.minimumMatchLength) {
						...
						// Star Logic
						// If there is a star tile in the match, add all tiles in row to match
						const match2 = [];
						if (match.some(tile => tile.pattern === TilePattern.Star)) {
							for (let x2 = 0; x2 < Board.SIZE; x2++) {
								match2.push(this.tiles[y][x2]);
							}
							rowMatches.push(match2);
							break;
						}	
						else{
			...
			if (matchCounter >= this.minimumMatchLength) {
				...
				if (match.some(tile => tile.pattern === TilePattern.Star)) {
					for (let x2 = 0; x2 < Board.SIZE; x2++) {
						match2.push(this.tiles[match[0].boardY][x2]);
					}
					rowMatches.push(match2);
				}	
				else{
	...
	resolveVerticalMatches() {
		for (let x = 0; x < Board.SIZE; x++) {
			...
			for (let y = 1; y < Board.SIZE; y++) {
				...
				if (this.tiles[y][x].colour === colourToMatch) {
					...
					if (matchCounter >= this.minimumMatchLength) {
						...
						// Star Logic
						// If their is a star tile in the match, add all tiles in row to match
						const match2 = [];
						if (match.some(tile => tile.pattern === TilePattern.Star)) {
							for (let y2 = 0; y2 < Board.SIZE; y2++) {
								match2.push(this.tiles[y2][x]);
							}
							columnMatches.push(match2);	
						}
						else{
			...
			if (matchCounter >= this.minimumMatchLength) {
				...
				if (match.some(tile => tile.pattern === TilePattern.Star)) {
					for (let y2 = 0; y2 < Board.SIZE; y2++) {
						match2.push(this.tiles[y2][match[0].boardX]);
					}
					columnMatches.push(match2);	
				}
				else{
