...
function update(dt) {
	...
	// AI for player 2
	// Keeps the ball in the middle of the paddle to make the ai more precise
	// Makes sure to only move when in game
	if (gameState === 'play') {
		// Checks if the ball is below the middle of the paddle.
		// In which case, the paddle will moves down.
		if (ball.y > player2.y + player2.height / 2) {
			player2.moveDown();
		}
		// Checks if the ball is above the middle of the paddle.
		// In which case, the paddle will moves up.
		else if (ball.y < player2.y + player2.height / 2) {
			player2.moveUp();
		}
		// If the ball is in the middle of the paddle, then no need to move
		else {
			player2.stop();
		}
	}
